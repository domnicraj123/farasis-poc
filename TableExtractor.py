...
class Table:
    ...
    def get_cell_span(self, target_cell):
        """
        Computes how many rows and columns a given cell spans.
        """
        row_span = col_span = 0
        for _, row in self.global_map.items():
            if target_cell in row.values():
                row_span += 1
        for col in zip(*self.global_map.values()):
            if target_cell in col:
                col_span += 1

        return row_span, col_span
    
    def find_merged_cells(self):
        """
        Finds merged cells and returns their location in terms of rows and columns.
        """
        merged_cells_info = []

        for row_index, row in self.global_map.items():
            for col_index, cell in row.items():
                row_span, col_span = self.get_cell_span(cell)
...
class TableExtractor:
    ...
    def parse_page(self, page_n,max_table_count):
        if self.debug:
        ...
        if len(tables.tables) > max_table_count:
            return []
...
def extract_merged_result(pdf_path, page):
    pdf_interpreter = TableExtractor(pdf_path)
    pdf_interpreter.draw = False  # Optional: for debugging and visualization
    pdf_interpreter.debug = False  # Optional: for debugging and visualization
    # Parse the page (assuming page index 0 for example)

    tables = pdf_interpreter.parse_page(page, 10)
    output_json = {}
    for i, table in enumerate(tables):
        output_json[i] = table.find_merged_cells()
    return output_json


if __name__ == '__main__':
    # Setup the PDF path and table extractor as before
    pdf_path = "/home/domni/tasks/py_pdf_stm/pdf/RFI_ZEV-BATTERY.pdf"
    output_file = "ouptput.txt"
    pdf_interpreter = TableExtractor(pdf_path)
    pdf_interpreter.draw = True  # Optional: for debugging and visualization
    pdf_interpreter.debug = True  # Optional: for debugging and visualization
    # Parse the page (assuming page index 0 for example)

    tables = pdf_interpreter.parse_page(4, 10)
    for table in tables:
        print(f"merged cells: ")
        output = table.find_merged_cells()
        print(output)
        print("-----------------")
    # Supposing the first found table is of interest
    #if tables:
    #    first_table = tables[0]

        # Optional: print cell texts for debugging purposes
